...
public class BusServiceImpl implements BusService {
    ...
    public List<ResultDTO> getBusTerminal(Long stationClass, String cityCode) {
        ...
        if (stationClass == 4L) { // 고속 버스 터미널
            urlBuilder = new ApiDefault(odsayConfig).getURLBuilder("/expressBusTerminals",0,odsayConfig.getKey());
        } else if (stationClass == 6L) {
            urlBuilder = new ApiDefault(odsayConfig).getURLBuilder("/intercityBusTerminals",0,odsayConfig.getKey());
        }
