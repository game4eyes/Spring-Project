...
public class AirInfoController {
    ...
    @Operation(
            summary = "항공 정보 조회",
            description = "출발지와 도착지 공항 ID, 시간, 요일을 기반으로 항공 정보를 조회합니다.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "항공 정보 조회 성공", content = @Content(schema = @Schema(implementation = ResultDTO.class))),
    ...
    public ResultDTO getAirInfo(
            @Parameter(description = "출발지 공항 ID", example = "3500001") @PathVariable String startStationID,
            @Parameter(description = "도착지 공항 ID", example = "3500003") @PathVariable String endStationID,
